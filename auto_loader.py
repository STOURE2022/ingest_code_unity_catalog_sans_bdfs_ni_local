...
class AutoLoaderModule:
    ...
    def __init__(self, spark, config):
        ...
        self.source_base_fs = self._to_fs_path(self.source_base)
    ...
    def process_all_tables(self, excel_config_path: str) -> dict:
        ...
        excel_path_fs = self._to_fs_path(excel_config_path)
        print(f"\n📖 Lecture configuration : {excel_config_path}")
        
        try:
            file_tables_df = pd.read_excel(excel_path_fs, sheet_name="File-Table")
            file_columns_df = pd.read_excel(excel_path_fs, sheet_name="Field-Column")
        ...
        for idx, trow in file_tables_df.iterrows():
            ...
            source_dir = os.path.join(self.source_base_fs, table_name)
    ...
    def _process_single_table(self, table_name: str, table_config, columns_config) -> dict:
        ...
        try:
            df_stream = (
                ...
                .format("cloudFiles")
                .options(**options)
                .load(source_path)  # Spark comprend le chemin Unity Catalog
            )
        except Exception as e:
            return {
                "status": "ERROR",
    ...
    def _add_metadata(self, df_stream):
        ...
        # Date depuis nom fichier (pattern : *_YYYYMMDD_*)
        df_stream = df_stream.withColumn(
            "yyyy",
            F.regexp_extract(F.col("FILE_NAME_RECEIVED"), r"_(\d{4})\d{6}", 1).cast("int")
        )
        
        df_stream = df_stream.withColumn(
            "mm",
            F.regexp_extract(F.col("FILE_NAME_RECEIVED"), r"_\d{4}(\d{2})\d{4}", 1).cast("int")
        )
