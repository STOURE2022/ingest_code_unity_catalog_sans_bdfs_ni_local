...
Databricks natif, plus de support DBFS ni local
"""
...
def main():
    """Point d'entrée pipeline - Unity Catalog uniquement"""
    ...
    for table_idx, trow in file_tables_df.iterrows():
        ...
        print(f"📂 Traitement de {len(files_to_read)} fichier(s)")
        ...
        for file_idx, (matched_uri, parts) in enumerate(files_to_read):
            print(f"\n📄 Fichier {file_idx + 1}/{len(files_to_read)} : {os.path.basename(matched_uri)}")
            ...
            try:
                ...
                logger_manager.log_execution(source_table, os.path.basename(matched_uri),
                                             input_format, ingestion_mode, output_zone,
                                             error_msg=f"Read error: {e}", status="FAILED",
                                             start_time=start_table_time)
                continue

            # Validation colonnes manquantes
            is_valid, err_df = validator.validate_columns_presence(
                df_raw, expected_cols, del_cols_allowed, source_table, os.path.basename(matched_uri)
            )
            ...
            # Ajouter colonnes manquantes avec NULL
            if expected_cols:
            ...
            df_raw, corrupt_rows, should_abort = file_processor.check_corrupt_records(
                df_raw, total_rows, rej_tol, source_table, os.path.basename(matched_uri)
            )
        ...
        if not df_raw_list:
            print(f"⚠️ Aucun DataFrame pour {source_table}")
            total_failed += 1
            continue

        if len(df_raw_list) > 1:
            print(f"🔗 Fusion de {len(df_raw_list)} DataFrames...")
        ...
        print(f"✅ DataFrame fusionné : {total_rows_initial} lignes")
        ...
        df_raw, col_errors, invalid_flags = column_processor.process_columns(
            ...
            filename=filename_for_log, total_rows=total_rows_initial
        )

        # Rejeter lignes invalides (ICT_DRIVEN)
        df_raw, line_errors = column_processor.reject_invalid_lines(
            df_raw, invalid_flags, table_name=source_table, filename=filename_for_log
        )
        ...
        df_err_global = validator.check_data_quality(
            df_raw, source_table, merge_keys, filename=filename_for_log, column_defs=file_columns_df
        )

        # ========== VALIDATION FINALE TYPES ==========

        df_raw, errors_list, column_errors = validator.validate_and_rebuild_dataframe(
            df_raw, column_defs_for_table, source_table, filename_for_log
        )
        ...
        try:
            ...
            logger_manager.log_execution(
                source_table, filename_for_log, input_format, ingestion_mode, output_zone,
                row_count=0, column_count=len(df_raw.columns),
                masking_applied=(output_zone == "secret"), error_msg=f"Ingestion error: {e}",
                status="FAILED", start_time=start_table_time
            )
            total_failed += 1
            continue
        ...
        logger_manager.print_summary(
            table_name=source_table, filename=filename_for_log,
            total_rows=(total_rows_initial, metrics["total_rows_after"]),
            corrupt_rows=metrics["corrupt_rows"], anomalies_total=metrics["anomalies_total"],
        ...
        logger_manager.log_execution(
            table_name=source_table, filename=filename_for_log, input_format=input_format,
            ingestion_mode=ingestion_mode, output_zone=output_zone,
            row_count=metrics["total_rows_after"], column_count=len(df_raw.columns),
            masking_applied=(output_zone == "secret"),
            error_msg=f"{metrics['anomalies_total']} errors detected" if metrics["anomalies_total"] > 0 else None,
            status="SUCCESS", error_count=metrics["anomalies_total"], start_time=start_table_time
        )

        total_success += 1
        print(f"\n✅ Table {source_table} traitée en {round(time.time() - start_table_time, 2)}s")
    ...
    if total_success > 0:
        ...
        print(f"   Format: <table_name>_all et <table_name>_last")
